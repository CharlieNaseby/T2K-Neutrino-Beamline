...
Interface::Interface(std::string dataFile, std::string baseBeamlineFile, 
  ...
  parametersMap = parametersMapIn;
  parametersVec = parametersVecIn;

  nPars = parametersVec->size();
  nMagnetPars = 0;
  nBeamPars = 0;
  for(auto &p : *parametersVec){
    if(p->type == "beam") nBeamPars++;
    else nMagnetPars++;
  }
  bds = new CNBDSIM();
...
void Interface::SetInitialValues(char *usePrevBestFit, bool useFieldMaps, bool useInputFile, double noise){
  ...
  for(auto &val : magCurrent){
    if(abs(val) < 1e-1)
      val = 1e-1; //annoyingly magnets with 0 strength create issues
  }

  //CERN TODO this is based on values generated by make_beamline.py and may not be reflective of the magnets in the run we're interested in
  //first set everything to the input gmad file
  for(auto &par : *parametersVec){
    par->nominalValue = bds->GetParameterValue(par);
    par->prefitValue = par->nominalValue;
    par->setPhysicalValue(par->nominalValue);
  }

  if(useFieldMaps){
    throw std::runtime_error("Using field maps for the fit CURRENTLY UNSUPPORTED");
  }
  ...
  if(usePrevBestFit){
    ...
    //CERN TODO in future we should save parameter name and be able to set only a subset based on a previous fit
    if(filePostFit.GetNrows() != parametersVec->size()){
      std::cerr << "invalid size of parameters supplied from previous fit file"<<std::endl;
      throw;
    }
    for(int i=0; i<filePostFit.GetNrows(); i++){
      (*parametersVec)[i]->setFitValue(filePostFit[i]);
      (*parametersVec)[i]->prefitValue = filePreFit[i];
      (*parametersVec)[i]->nominalValue = fileNominal[i];
    }
    inf.Close();
  }
  ...
  for(auto &par : *parametersVec){
    if(par->getState() == "free") par->setFitValue(par->getFitValue() * rand.Gaus(1.0, noise));
    std::cout << par->name << " is set to " << par->getFitValue() << std::endl;
  }
...
void Interface::ParamScan(std::string param, TH1D *hist){
  double store = parametersMap->at(param)->getFitValue(); 
  for(int i=1; i<=hist->GetNbinsX(); i++){
    double xpoint = hist->GetBinCenter(i);
    parametersMap->at(param)->setFitValue(xpoint);
    hist->SetBinContent(i, fcn());
  }
  parametersMap->at(param)->setFitValue(store);
}
...
double Interface::fcn(){
  std::vector<double> pars;
  for(auto &par : *parametersVec){
    pars.push_back(par->getFitValue());
  }
  return fcn(pars);
}
...
double Interface::CalcPrior(std::map<std::string, double> pars){
  ...
  for(auto &par : *parametersVec){
    if(par->priorError > 0){
      double contribution = std::pow(pars[par->name] - par->nominalValue, 2)/(std::pow(par->priorError,2)); //before you efficiency freaks complain pow(x, 2) expands to x*x with -O1+
      chisq += contribution;
    }
    else if(par->priorError < 0){
      double contribution = std::pow(pars[par->name] - par->nominalValue, 2)/(std::pow(par->nominalValue*par->priorError, 2));
      chisq += contribution;
    }
  }
  return chisq;
...
//  beamline.resize(1);
  ...
  for(auto &par : *parametersVec){
    if(par->type == "beamPosition" || par->type == "beamTwiss") matchStrings.push_back(par->name+'=');
  }

  int filePars=0;
  while(std::getline(infile, line)){
    //most of this is deprecated actually all we need to do is check the number of parameters matches
    for(auto name : matchStrings){
      if(line.find(name) != std::string::npos && line.find("bellows") == std::string::npos){ //if we found a match and its not a bellows
        filePars++;
      }
    }
  }
...
//      if(labstart != std::string::npos && line.find("bellows") == std::string::npos){ //if we found a match and its not a bellows
//        //CERN TODO this doesn't work for arc magnets which are combined function
...
//  }
  ...
  if(nPars != filePars){
    ...
    std::cerr<<"Expected "<<nPars<<" but file has "<<filePars<<std::endl;
    throw;
  }
}
...
double Interface::FitToPhysical(std::string name, double fitval){
  for(auto &par : *parametersVec){
    if(par->name == name) return fitval*par->prefitValue;
  }
  return -999;
}
...
double Interface::PhysicalToFit(std::string name, double physval){
  for(auto &par : *parametersVec){
    if(par->name == name) return physval/par->prefitValue;
  }
  return -999;
}
...
std::map<std::string, double> Interface::GetParMap(const double *pars){
  std::map<std::string, double> parmap;
  for(size_t i=0; i<parametersVec->size(); i++){
    auto &par = parametersVec->at(i);
    parmap[par->name] = FitToPhysical(par->name, pars[i]);
  }
  return parmap;
}
...
//    std::cout<<"SSEM "<<i+1<<" position cumulative chisq contribution "<<chisq<<std::endl;
  ...
  std::cout<<"returning chisq = "<<chisq<<" of which prior contributes "<<prior<<" in fitmode "<<fitMode<<std::endl;
